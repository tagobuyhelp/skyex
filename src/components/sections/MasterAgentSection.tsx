...
export const MasterAgentSection = ({
  ...
  return <section className="container py-6 md:py-12 px-4">
      <div className="glass-card p-4 md:p-6">
        <h2 className="md:text-3xl font-semibold mb-4 flex items-center gap-2 text-lg">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {agent.agent_contacts[0]?.whatsapp && <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-base md:text-lg">{formatPhoneNumber(agent.agent_contacts[0].whatsapp)}</span>
                      </div>}
                  </div>
                </div>
                <div className="flex gap-2 self-end sm:self-center">
                  <button className="px-3 py-2 hover:bg-emerald-500/20 rounded-lg transition-colors flex items-center gap-1.5 whitespace-nowrap text-sm md:text-base" onClick={() => onViewHierarchy(agent)}>
                    <Eye className="w-4 h-4 md:w-5 md:h-5 text-emerald-400" />
                    <span className="text-emerald-400">দেখুন</span>
                  </button>
                  <button className="px-3 py-2 hover:bg-red-500/20 rounded-lg transition-colors flex items-center gap-1.5 whitespace-nowrap text-sm md:text-base" onClick={() => onComplaint(agent)}>
