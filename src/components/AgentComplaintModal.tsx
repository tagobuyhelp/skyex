        ...
        <div className="space-y-5">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              <button className={`px-2 py-1.5 rounded-lg text-xs sm:text-sm flex items-center justify-center gap-1.5 ${complaintType === 'selected' ? 'bg-amber-500/20 text-amber-500 border border-amber-500/40' : 'bg-gray-800/50 text-gray-400 border border-gray-700'}`} onClick={() => setComplaintType('selected')}>
                <Flag className="w-3 h-3 sm:w-4 sm:h-4" />
                এই এজেন্ট
              </button>
              
              <button className={`px-2 py-1.5 rounded-lg text-xs sm:text-sm flex items-center justify-center gap-1.5 ${complaintType === 'upline' ? 'bg-amber-500/20 text-amber-500 border border-amber-500/40' : 'bg-gray-800/50 text-gray-400 border border-gray-700'}`} onClick={() => {
              if (uplineAgent) {
              ...
              <button className={`px-2 py-1.5 rounded-lg text-xs sm:text-sm flex items-center justify-center gap-1.5 ${complaintType === 'site_admin' ? 'bg-amber-500/20 text-amber-500 border border-amber-500/40' : 'bg-gray-800/50 text-gray-400 border border-gray-700'}`} onClick={() => setComplaintType('site_admin')}>
                <Flag className="w-3 h-3 sm:w-4 sm:h-4" />
                সাইট এডমিন
              </button>
            </div>
          ...
          {complaintType === 'selected' && <div className="space-y-3">
              <h3 className="text-xs sm:text-sm text-gray-400">এজেন্টের তথ্য</h3>
              <div className="bg-gray-800/50 p-3 sm:p-4 rounded-lg">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-white font-medium text-sm sm:text-base bg-lime-600">
          ...
          {/* Upline Agent Info */}
          {complaintType === 'upline' && uplineAgent && <div className="space-y-3">
          ...
          {complaintType === 'site_admin' && <div className="space-y-3">
              ...
              {siteAdmins.length > 0 ? <>
                  <div className="grid grid-cols-1 gap-2 max-h-[120px] overflow-y-auto">
                    {siteAdmins.map(admin => <button key={admin.id} className={`text-left p-3 rounded-lg flex items-center gap-3 ${selectedSiteAdmin?.id === admin.id ? 'bg-amber-500/20 border border-amber-500/40' : 'bg-gray-800/50 border border-gray-700'}`} onClick={() => setSelectedSiteAdmin(admin)}>
                        ...
                        {selectedSiteAdmin?.id === admin.id && <Check className="w-4 h-4 text-amber-500 ml-auto" />}
                      </button>)}
                  </div>
                  
                  {selectedSiteAdmin && <div className="mt-2">
                      {selectedSiteAdmin.agent_contacts[0]?.whatsapp && <a href={`https://wa.me/${selectedSiteAdmin.agent_contacts[0].whatsapp}`} className="inline-flex items-center gap-1.5 text-xs sm:text-sm text-emerald-400 hover:text-emerald-300" target="_blank" rel="noopener noreferrer">
                          <WhatsAppIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
          ...
          <div className="space-y-3">
            <div>
              ...
              <input id="whatsapp" type="text" value={whatsapp} onChange={e => setWhatsapp(e.target.value)} className="w-full bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-xs sm:text-sm text-white placeholder-gray-500" placeholder="কান্ট্রি কোড সহ লিখুন (যেমন: +8801234567890)" />
            </div>
            ...
            <div>
              ...
              <textarea id="complaint" rows={4} value={complaint} onChange={e => setComplaint(e.target.value)} className="w-full bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-xs sm:text-sm text-white placeholder-gray-500 resize-none" placeholder="রিপোর্টের কারণ বিস্তারিত লিখুন..." />
            </div>
          </div>

          {/* Action Buttons */}
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={() => onOpenChange(false)} className="px-3 py-1.5 rounded-lg bg-gray-800 text-xs sm:text-sm text-gray-300 hover:bg-gray-700 transition-colors" disabled={isSubmitting}>
              বাতিল করুন
            </button>
            <button onClick={handleSubmit} disabled={isSubmitting} className="px-3 py-1.5 rounded-lg bg-red-500 text-xs sm:text-sm text-white hover:bg-red-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5">
